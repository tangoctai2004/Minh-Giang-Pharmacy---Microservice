/**
 * validate.js — Tập hợp middleware validate đầu vào cho CMS Service.
 */
...
/** Validate các trường bắt buộc phải có trong req.body */
...
function validateEnum(field, allowedValues = []) {
  return function validateEnumField(req, res, next) {
    ...
    if (!allowedValues.includes(value)) {
      return badRequest(res, `${field} không hợp lệ. Giá trị cho phép: ${allowedValues.join(', ')}`);
    }
    return next();
  };
}

/** Validate một trường số trong phạm vi [min, max] */
...
/** Validate khoảng thời gian: startField < endField */
function validateDateWindow(startField, endField) {
  return function validateDateWindowFields(req, res, next) {
    ...
    if (startDate >= endDate) {
      return badRequest(res, `${startField} phải nhỏ hơn ${endField}`);
    }
    return next();
  };
}
