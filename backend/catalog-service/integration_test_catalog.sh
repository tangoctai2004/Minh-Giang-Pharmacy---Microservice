...
R="$(curl -sS -X POST "$BASE/batches" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d '{"supplier_id":1,"received_date":"2026-04-22","status":"draft","items":[{"product_id":1,"lot_number":"LOT-INT-001","expiry_date":"2028-12-31","quantity_received":5,"cost_price":1000}]}' )"
...
R="$(curl -sS -X PUT "$BASE/batches/$BATCH_ID" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d "{\"status\":\"completed\",\"items\":[{\"id\":$ITEM_ID,\"product_id\":1,\"lot_number\":\"LOT-INT-001-UPD\",\"expiry_date\":\"2028-12-31\",\"quantity_received\":6,\"quantity_remaining\":6,\"cost_price\":1100}]}" )"
assert_success "Flow4 - update draft batch" "$R"
R="$(curl -sS -X PUT "$BASE/batches/$BATCH_ID" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d "{\"status\":\"draft\"}")"
assert_fail "Flow4 - reject update after completed" "$R"

# Flow 5: Promotions validate/toggle/reset/loyalty
R="$(curl -sS -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" "$BASE/promotions/vouchers?page=1&limit=1")"
...
R="$(curl -sS -X PUT "$BASE/promotions/vouchers/$VID/toggle" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d '{"status":"active"}')"
...
R="$(curl -sS -X POST "$BASE/promotions/vouchers/$VID/consume" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d '{"quantity":1,"idempotency_key":"integration-consume-001"}')"
...
R="$(curl -sS -X PUT "$BASE/promotions/vouchers/$VID/toggle" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d '{"status":"paused"}')"
assert_success "Flow5 - toggle voucher paused" "$R"
R="$(curl -sS -X PUT "$BASE/promotions/vouchers/$VID/reset-usage" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN")"
assert_success "Flow5 - reset voucher usage" "$R"
R="$(curl -sS -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" "$BASE/promotions/loyalty/config")"
