  ...
  try {
    ...
    const [rows] = await pool.query(
      `SELECT id, code, name, discount_type, discount_value, max_discount,
              min_order_amount, valid_from, valid_to, status
    ...
    const data = rows.map((voucher) => {
      ...
      let title = voucher.name;
      let description = voucher.code;
      ...
      if (voucher.discount_type === 'percent') {
        ...
        description = maxDiscount > 0
          ? `Toi da ${maxDiscount.toLocaleString('vi-VN')} VND`
          : `Giam ${discountPercent}%`;
      } else if (voucher.discount_type === 'fixed') {
        bannerColor = 'orange';
        description = `Giam ${discountValue.toLocaleString('vi-VN')} VND`;
      ...
      if (minOrder > 0) {
        description += ` cho don tu ${minOrder.toLocaleString('vi-VN')} VND`;
      }
      ...
      return {
        ...
        tag,
        cta_url: `/category.html?promo=${voucher.id}`,
