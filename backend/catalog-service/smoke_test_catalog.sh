...
R="$(curl -sS -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" "$BASE/promotions/stats")"
check_success_true "GET /promotions/stats (admin)" "$R"

R="$(curl -sS -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" "$BASE/promotions/vouchers?page=1&limit=5")"
...
R="$(curl -sS -X POST "$BASE/promotions/vouchers/$VID/consume" -H "x-user-role: admin" -H "x-gateway-token: $GW_TOKEN" -H "Content-Type: application/json" -d '{"quantity":1,"idempotency_key":"smoke-consume-001"}')"
